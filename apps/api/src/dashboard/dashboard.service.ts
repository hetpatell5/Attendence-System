...
export class DashboardService {
  ...
  async getAdminDashboard() {
    ...
    const [
      ...
      this.prisma.salaryRecord.findMany({ where: { status: 'PENDING' }, include: { employee: true } }),
      this.prisma.salaryRecord.findMany({ where: { status: 'PAID' }, include: { employee: true } }),
    ...
    return {
      ...
      pendingLeaveCount,
      pendingSalaryCount: pendingSalaryCount.length,
      paidSalaryCount: paidSalaryCount.length,
      totalShifts,
      ...
      paidList: paidSalaryCount,
      unpaidList: pendingSalaryCount,
